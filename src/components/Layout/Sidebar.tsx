...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      <aside className="w-72 bg-pd-surface/95 border-r border-pd-b1 flex flex-col shrink-0 backdrop-blur-xl">
        <div className="p-4 border-b border-pd-b1">
          <button
            onClick={() => setShowAddModal(true)}
            className="w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg border border-violet-500/40 bg-violet-600/15 text-violet-100 hover:bg-violet-600/25 hover:border-violet-400/60 text-xs font-semibold tracking-wide uppercase transition-all duration-150 shadow-glow-violet-sm"
          >
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-3 space-y-1">
          {loading ? (
            ...
            <>
              <div className="px-2 py-1.5 text-[10px] font-semibold text-slate-500 uppercase tracking-[0.18em]">
                Library
              </div>
              {games.map((game) => (
                <div
                  ...
                  className={`group relative flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-150 border ${
                    selectedGameId === game.id
                      ? 'bg-violet-600/15 border-violet-500/30 text-white shadow-glow-violet-sm'
                ...
                >
                  ...
                  />
                  <span className={`text-xs font-bold truncate flex-1 ${selectedGameId === game.id ? 'text-white' : ''}`}>
                    {game.name}
                  </span>
